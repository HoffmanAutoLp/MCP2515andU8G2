...
MCP_CAN CAN0(4);                               // Set CS to pin 10 <--------- CHANGE if using different pin number
    ...
    switch (__pid) {
      case PID_COOLANT_TEMP:
        if(rxBuf[2] == PID_COOLANT_TEMP){
          uint8_t temp;
          temp = rxBuf[3] - 40;
          int tempFahrenheit = (int)((temp * 9.0 / 5.0) + 32.0); // Convert to Fahrenheit and cast to int
          Serial.print("ECT ");
          Serial.println(tempFahrenheit); // Print temperature in Fahrenheit as a whole number
        }
      break;
...
void loop()
{
  ...
  delay(15); //to allow time for ECU to reply
